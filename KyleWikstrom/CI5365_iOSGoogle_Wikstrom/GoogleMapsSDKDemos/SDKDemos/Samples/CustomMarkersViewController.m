...
- (void)viewDidLoad {
  [super viewDidLoad];
  GMSCameraPosition *camera = [GMSCameraPosition cameraWithLatitude:46.435642
                                                          longitude:-94.292571
                                                               zoom:13];
...
- (void)addDefaultMarkers {
  ...
  sydneyMarker.title = @"Iven's Bay, North Long Lake";
  sydneyMarker.icon = [UIImage imageNamed:@"glow-marker"];
  sydneyMarker.position = CLLocationCoordinate2DMake(46.435642, -94.292571);
  sydneyMarker.map = mapView_;

  // Add a custom 'arrow' marker pointing to Melbourne.
  GMSMarker *melbourneMarker = [[GMSMarker alloc] init];
  melbourneMarker.title = @"Timberwood Church";
  melbourneMarker.icon = [UIImage imageNamed:@"arrow"];
  melbourneMarker.position = CLLocationCoordinate2DMake(46.486058, -94.294871);
